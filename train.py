...
def train():
    ...
    # ── 5b-ii. Log gate / entity_mem_scale diagnostics ───────────────────────
    with torch.no_grad():
        _gb  = model.graph_text_fusion.gate_proj.bias.data.mean().item()
        _ems = model.entity_mem_scale.data.item()
        import math as _math
        print(f"  gate_proj.bias mean={_gb:.3f}  "
              f"(sigmoid≈{1/(1+_math.exp(-_gb)):.4f} graph weight)")
        print(f"  entity_mem_scale={_ems:.4f}  (tanh≈{_math.tanh(_ems):.4f})")
